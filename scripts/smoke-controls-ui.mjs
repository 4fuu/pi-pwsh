// ConPTY integration test with a scripted HTTP provider; no external model requests.
...
const command = `Write-Output TEST_STARTED; Set-Content -LiteralPath ${quote(readyPath)} -Value $PID; Start-Sleep -Seconds 120`;
...
const server = createServer(async (req, res) => {
	try {
		...
		const delta = first
			? { role: "assistant", tool_calls: [{ index: 0, id: "test_call", type: "function", function: { name: "pwsh", arguments: JSON.stringify({ command, wait: 120 }) } }] }
			: { role: "assistant", content: "TEST_DIALOG_AVAILABLE" };
		res.writeHead(200, { "Content-Type": "text/event-stream" });
		const event = (delta, finish_reason) => `data: ${JSON.stringify({ id: "test", object: "chat.completion.chunk", created: 1, model: "test", choices: [{ index: 0, delta, finish_reason }] })}\n\n`;
...
export default function(pi) {
  ...
  pi.registerCommand = (name, options) => registerCommand(name, name === "tasks" ? {
    ...options, handler: async (...args) => {
      ...
      args[1].ui.notify("TEST_TASKS_CLOSED", "info");
    }
  } : options);
  ...
  pi.registerShortcut = (key, options) => registerShortcut(key, key === "ctrl+alt+t" ? {
    ...options, handler: async ctx => {
      ...
      ctx.ui.notify("TEST_SHORTCUT_CLOSED", "info");
    }
  } : options);
  pi.registerCommand("test-prompt-ready", { handler: async (_args, ctx) => ctx.ui.notify("TEST_PROMPT_READY", "info") });
  ...
  pwsh(pi);
  pi.on("session_start", (_e, ctx) => { setTimeout(() => ctx.ui.notify("TEST_SESSION_READY", "info"), 1000); });
  pi.registerProvider("test-local", {
    baseUrl: "http://127.0.0.1:${port}/v1", apiKey: "local-test-only", api: "openai-completions",
    models: [{ id: "test", name: "Task controls test", reasoning: false, input: ["text"],
...
try {
	...
	terminal = pty.spawn(runtime, [cli, "--no-extensions", "--no-skills", "--no-prompt-templates", "--no-themes", "-e", fixture, "--provider", "test-local", "--model", "test", "--thinking", "off"], {
	...
	await until(() => text().includes("TEST_SESSION_READY"), "Pi startup", 60000);
	send("Start the test task\r");
	...
	send("\x1b");
	await until(() => text().includes("TEST_SHORTCUT_CLOSED"), "close task shortcut without aborting foreground wait");
	output = "";
	console.log("PASS Ctrl+Alt+T during a foreground wait");
	send("\x1b\x02"); // Ctrl+Alt+B in legacy terminal encoding.
	await until(() => text().includes("TEST_DIALOG_AVAILABLE") && !!taskId, "foreground detach and model continuation");
	process.kill(processPid, 0);
	...
	output = "";
	send("\r");
	await until(() => text().includes("log:") && text().includes("TEST_STARTED"), "task output inspection");
	...
	await until(() => text().includes("TEST_TASKS_CLOSED"), "last active stop closes viewer");
	output = "";
	send("/test-prompt-ready\r");
	await until(() => text().includes("TEST_PROMPT_READY"), "prompt editor accepts commands after stop");
	output = "";
